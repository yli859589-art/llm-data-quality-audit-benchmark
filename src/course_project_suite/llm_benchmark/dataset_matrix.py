...
def _write_summary(output_dir: Path, rows: list[DatasetMatrixResult]) -> None:
    ...
    lines = [
        ...
        (
            "Interpretation: local trained rows demonstrate executable dataset "
            "matrix runs; fallback rows record unavailable optional datasets."
        ),
    ...
    *,
    ...
    for dataset_key in dataset_keys:
        ...
        try:
            ...
            status = "paper_prototype_small_run" if mode == "paper-prototype" else "ok"
            payload.update(
                {
                    "dataset_key": dataset_key,
                    "status": status,
            ...
            _write_json(output_dir / dataset_key / "results.json", payload)
