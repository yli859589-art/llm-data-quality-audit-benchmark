...
class AlgorithmEdgeTests(unittest.TestCase):
    ...
    def test_q_learning_update_uses_bootstrap_value(self):
        agent = QLearningAgent(["left", "right"], alpha=0.5, epsilon=0.0, gamma=0.9)
    ...
    def test_particle_filter_concentrates_on_likely_state(self):
        random.seed(4)
        likelihood = lambda observation, state: 0.95 if state == observation else 0.05
        filt = ParticleFilter([0, 1], lambda state: state, likelihood, num_particles=600)
        filt.observe(1)
        self.assertGreater(filt.belief().get(1, 0.0), 0.85)
    ...
    def test_rnn_step_gradient(self):
        ...
        prev = np.random.randn(2, 4)
        wx = np.random.randn(3, 4)
        wh = np.random.randn(4, 4)
        b = np.random.randn(4)
        dout = np.random.randn(2, 4)
        _, cache = rnn_step_forward(x, prev, wx, wh, b)
        dx, dprev, dwx, dwh, db = rnn_step_backward(dout, cache)
        ...
        self.assertEqual(dwx.shape, wx.shape)
        self.assertEqual(dwh.shape, wh.shape)
