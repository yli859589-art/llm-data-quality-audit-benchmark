...
class LlmBenchmarkTests(unittest.TestCase):
    ...
    def test_attention_suite_matches_naive(self):
        ...
        for row in rows:
            ...
            self.assertIn("median_seconds_per_call", row)
            self.assertIn("p25_seconds_per_call", row)
            self.assertIn("p75_seconds_per_call", row)

    def test_svg_reporters_write_renderable_files(self):
        with tempfile.TemporaryDirectory() as tmp:
            line = Path(tmp) / "line.svg"
            bars = Path(tmp) / "bars.svg"
