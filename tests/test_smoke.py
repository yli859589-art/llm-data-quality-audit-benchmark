...
from course_project_suite.cs231n.layers import affine_forward, affine_backward, conv_forward_naive, conv_backward_naive, batchnorm_forward, batchnorm_backward
...
class SmokeTests(unittest.TestCase):
    ...
    def test_affine_gradient(self):
        np.random.seed(0); x=np.random.randn(4,3); w=np.random.randn(3,5); b=np.random.randn(5); dout=np.random.randn(4,5)
        out,cache=affine_forward(x,w,b); dx,dw,db=affine_backward(dout,cache)
    ...
    def test_conv_gradient(self):
        np.random.seed(1); x=np.random.randn(1,1,4,4); w=np.random.randn(1,1,3,3); b=np.random.randn(1); dout=np.random.randn(1,1,4,4)
        out,cache=conv_forward_naive(x,w,b,{'stride':1,'pad':1}); dx,dw,db=conv_backward_naive(dout,cache)
    ...
    def test_attention_matches_sdpa(self):
        torch.manual_seed(0); q=torch.randn(1,2,8,4); k=torch.randn_like(q); v=torch.randn_like(q)
        a=naive_attention(q,k,v); b=torch_sdpa_attention(q,k,v)
        self.assertLess(float((a-b).abs().max()),1e-5)

    def test_kmeans_labels_match_final_centers(self):
        rng=np.random.default_rng(123); X=rng.normal(size=(8,2))
    ...
    def test_adamw_closure_allows_gradients(self):
        p=torch.tensor([1.0],requires_grad=True); opt=AdamW([p],lr=1e-3)
        def closure():
            opt.zero_grad(); loss=(p**2).sum(); loss.backward(); return loss
    ...
    def test_forward_filter_normalizes_each_step(self):
        ...
        for belief in history: self.assertAlmostEqual(sum(belief.values()),1.0)
